...
exports.getTermsByCouponId = async (req, res, next) => {
    try {
        ...
        let TermsAndConditions = {
            ...
            2: `Instant Discount of ${discount_type === 'fixed' ? `$${discount_value}` : `${discount_value}%`} ${max_discount !== null ? 'Up to $' + max_discount : ''}`,
            ...
            5: `Minimum Transaction value of $${min_price}`
        }
    ...
    try {
        ...
        await Promise.all(
            parsedProducts.map(async (product) => {
                ...
                order_sub_total += parseFloat(ProductQuantity[0].price) * parseFloat(product.quantity)
            })
        );
